...
function Atr_FindTabIndex(whichTab)
    if (_AUCTIONATOR_SELL_TAB_INDEX == 0) then
        ...
        while (true) do
            ...
            if (tab.auctionatorTab) then
                if (tab.auctionatorTab == SELL_TAB) then
                    _AUCTIONATOR_SELL_TAB_INDEX = i
                end
                if (tab.auctionatorTab == MORE_TAB) then
                    _AUCTIONATOR_MORE_TAB_INDEX = i
                end
                if (tab.auctionatorTab == BUY_TAB) then
                    _AUCTIONATOR_BUY_TAB_INDEX = i
                end
            end
    ...
    if (whichTab == SELL_TAB) then
        return _AUCTIONATOR_SELL_TAB_INDEX
    end
    if (whichTab == MORE_TAB) then
        return _AUCTIONATOR_MORE_TAB_INDEX
    end
    if (whichTab == BUY_TAB) then
        return _AUCTIONATOR_BUY_TAB_INDEX
    end
...
function Atr_OnAuctionMultiSellUpdate()
    ...
    if (stacksSoFar == stacksTotal) then
        ...
        Atr_AddHistoricalPrice(gJustPosted_ItemName, gJustPosted_BuyoutPrice / gJustPosted_StackSize,
            gJustPosted_StackSize, gJustPosted_ItemLink)
    end
end
...
function Atr_OnAuctionMultiSellFailure()
    ...
    Atr_AddHistoricalPrice(gJustPosted_ItemName, gJustPosted_BuyoutPrice / gJustPosted_StackSize, gJustPosted_StackSize,
        gJustPosted_ItemLink)
...
function Atr_ResetDuration()
    ...
    if (AUCTIONATOR_DEF_DURATION == "M") then
        UIDropDownMenu_SetSelectedValue(Atr_Duration, 2)
    end
    if (AUCTIONATOR_DEF_DURATION == "L") then
        UIDropDownMenu_SetSelectedValue(Atr_Duration, 3)
    end
end
...
function Atr_ShowingSearchSummary()
    if (gCurrentPane.activeSearch and gCurrentPane.activeSearch.searchText ~= "" and gCurrentPane:IsScanEmpty() and
...
function Atr_UpdateRecommendation(updatePrices)
    ...
    elseif (gCurrentPane.activeScan.absoluteBest and basedata.stackSize ==
        gCurrentPane.activeScan.absoluteBest.stackSize and basedata.buyoutPrice ==
        gCurrentPane.activeScan.absoluteBest.buyoutPrice) then
...
function Atr_UpdateUI_SellPane(needsUpdate)
    ...
    if (needsUpdate) then
        ...
        if (gJustPosted_ItemName ~= nil) then
            ...
            gCurrentPane.currIndex = gCurrentPane.activeScan:FindInSortedData(gJustPosted_StackSize,
                gJustPosted_BuyoutPrice)
...
function Atr_HighlightEntry(entryIndex)
    ...
    if (Atr_ShowingCurrentAuctions() and entryIndex ~= nil and entryIndex > 0 and entryIndex <=
...
function Atr_CancelAuction_ByIndex(index)
    ...
    while (true) do
        local name, texture, count, quality, canUse, level, minBid, minIncrement, buyoutPrice, bidAmount, highBidder,
            owner = GetAuctionItemInfo("owner", i)

...
function Atr_Buy_Debug1(...)
    ...
    if (gBuyState == ATR_BUY_QUERY_SENT) then
        asstr = "ATR_BUY_QUERY_SENT"
    end
    if (gBuyState == ATR_BUY_CHECKING_PAGE_FOR_MATCHES) then
        asstr = "ATR_BUY_CHECKING_PAGE_FOR_MATCHES"
    end
    if (gBuyState == ATR_BUY_JUST_BOUGHT) then
        asstr = "ATR_BUY_JUST_BOUGHT"
    end
    if (gBuyState == ATR_BUY_WAITING_FOR_AH_CAN_SEND) then
        asstr = "ATR_BUY_WAITING_FOR_AH_CAN_SEND"
    end
    if (gBuyState == ATR_BUY_PAGE_WITH_ITEM_LOADED) then
        asstr = "ATR_BUY_PAGE_WITH_ITEM_LOADED"
    end
...
function Atr_Buy_CountMatches(andBuy)
    ...
    while (true) do
        ...
        if (zc.StringSame(name, gAtr_Buy_ItemName) and buyoutPrice == gAtr_Buy_BuyoutPrice and count ==
            gAtr_Buy_StackSize) then
            ...
            if (andBuy and gAtr_Buy_NumUserWants > gAtr_Buy_NumBought) then
                ...
                Atr_Buy_Continue_Text:SetText(string.format(ZT("%d of %d bought so far"), gAtr_Buy_NumBought,
                    gAtr_Buy_NumUserWants))
            end
        end

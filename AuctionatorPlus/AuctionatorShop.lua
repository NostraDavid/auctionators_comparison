...
function Atr_SList:DisplayX()
    ...
    for line = 1, SLITEMS_NUM_LINES do
        ...
        if (dataOffset <= numrows and slItem) then
            ...
            if (currentPane.activeSearch.origSearchText ~= "" and
                zc.StringSame(slItem, currentPane.activeSearch.origSearchText)) then
...
function Atr_Shop_UpdateUI()
    ...
    elseif (gTempShoppingList) then
        local listWithSameName = Atr_SList.FindByName(gTempShoppingList.name, {
            skipTempList = true
        })

        if (gTempShoppingList == currentPane.activeSearch.shplist and #gTempShoppingList.items > 1 and listWithSameName ==
            nil) then
...
function Atr_Adv_Search_Onclick()
    ...
    if (Atr_IsCompoundSearch(searchText)) then
        local queryString, itemClass, itemSubclass, minLevel, maxLevel, minItemLevel, maxItemLevel, qualityIndex =
...
function Atr_Adv_Search_Do()
    ...
    if (zc.StringContains(itemRarity, "-")) then
        itemRarity = nil
    end
    ...
    if (minLevel > 0) then
        searchText = searchText .. "/" .. minLevel
    end
    if (maxLevel > 0) then
        searchText = searchText .. "/" .. maxLevel
    end
    ...
    if (itemClass and (itemClass == WEAPON or itemClass == ARMOR)) then
        ...
        if (minItemLevel > 0) then
            searchText = searchText .. "/i" .. minItemLevel
        end
        if (maxItemLevel > 0) then
            searchText = searchText .. "/i" .. maxItemLevel
        end
    end
    ...
    if (itemRarity ~= nil) then
        searchText = itemRarity .. "/" .. searchText
    end
...
StaticPopupDialogs["ATR_SL_REQUEST_SHARING"] = {
    ...
    OnShow = function(self)
        local s = string.format(ZT("|cffffbb00%s|r\nwould like to share Auctionator shopping lists."),
            gShpListShareRequester)
        self.text:SetText(s)
    end,
    timeout = 20,
    exclusive = 1,
...
function Atr_Update_ShareRequest()
    ...
    zc.msg_anm("|cffffbb00", gSLpermittedUser,
        "|ris either not an Auctionator user or is running a version that doesn't support sharing")

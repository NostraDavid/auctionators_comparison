...
function Atr_SetupOptionsFrame()
    local expText = "<html><body>" .. "<p>" .. ZT("The latest information on Auctionator can be found at") ..
                        " auctionator-addon.com." .. "</p>" .. "<p><br/>" ..
                        "MoP disenchanting data courtesy of the Norganna's AddOns (the Auctioneer folks)" .. "</p>" ..
                        "<p><br/>" .. "|cffaaaaaa" ..
                        string.format(ZT("German translation courtesy of %s"), "|rCkaotik") .. "<br/>" .. "|cffaaaaaa" ..
                        string.format(ZT("Russian translation courtesy of %s"), "|rStingerSoft") .. "<br/>" ..
                        "|cffaaaaaa" .. string.format(ZT("Swedish translation courtesy of %s"), "|rHellManiac") ..
                        "<br/>" .. "|cffaaaaaa" .. string.format(ZT("French translation courtesy of %s"), "|rKiskewl") ..
                        "<br/>" .. "|cffaaaaaa" .. string.format(ZT("Spanish translation courtesy of %s"), "|rElfindor") ..
                        "<br/>" .. "|cffaaaaaa" ..
                        string.format(ZT("Chinese/Taiwan translation courtesy of %s"), "|rScars") .. "<br/>" .. "</p>" ..
                        "</body></html>"
...
function Atr_BasicOptionsFrame_Save(frame)
    ...
    local origValues = zc.msg_str(AUCTIONATOR_ENABLE_ALT, AUCTIONATOR_OPEN_ALL_BAGS, AUCTIONATOR_SHOW_ST_PRICE,
        AUCTIONATOR_DEFTAB, AUCTIONATOR_DEF_DURATION)
    ...
    if (Atr_RB_S:GetChecked()) then
        AUCTIONATOR_DEF_DURATION = "S"
    end
    if (Atr_RB_M:GetChecked()) then
        AUCTIONATOR_DEF_DURATION = "M"
    end
    if (Atr_RB_L:GetChecked()) then
        AUCTIONATOR_DEF_DURATION = "L"
    end

    local newValues = zc.msg_str(AUCTIONATOR_ENABLE_ALT, AUCTIONATOR_OPEN_ALL_BAGS, AUCTIONATOR_SHOW_ST_PRICE,
        AUCTIONATOR_DEFTAB, AUCTIONATOR_DEF_DURATION)
...
function Atr_TooltipsOptionsFrame_Save(frame)
    ...
    local origValues = zc.msg_str(AUCTIONATOR_V_TIPS, AUCTIONATOR_A_TIPS, AUCTIONATOR_D_TIPS, AUCTIONATOR_SHIFT_TIPS,
        AUCTIONATOR_DE_DETAILS_TIPS)
    ...
    local newValues = zc.msg_str(AUCTIONATOR_V_TIPS, AUCTIONATOR_A_TIPS, AUCTIONATOR_D_TIPS, AUCTIONATOR_SHIFT_TIPS,
        AUCTIONATOR_DE_DETAILS_TIPS)
...
kStackList_categories[ATR_SK_GLYPHS] = {
    txt = ZT("Glyphs")
}
kStackList_categories[ATR_SK_GEMS_CUT] = {
    txt = ZT("Gems - Cut")
}
kStackList_categories[ATR_SK_GEMS_UNCUT] = {
    txt = ZT("Gems - Uncut")
}
kStackList_categories[ATR_SK_ITEM_ENH] = {
    txt = ZT("Item Enhancements")
}
kStackList_categories[ATR_SK_POT_ELIX] = {
    txt = ZT("Potions and Elixirs")
}
kStackList_categories[ATR_SK_FLASKS] = {
    txt = ZT("Flasks")
}
kStackList_categories[ATR_SK_HERBS] = {
    txt = ZT("Herbs")
}
...
function Atr_StackingList_Display()
    ...
    for line = 1, kStackList_LinesToDisplay do
        ...
        if (dataOffset <= totalRows and plist[dataOffset]) then
            ...
            if (numstacks == -2) then
                info = "|cff777777" .. ZT("default behavior")
            elseif (numstacks == -1) then
                info = string.format(ZT("max. stacks of %d"), stacksize)
            elseif (stacksize == 0) then
                info = "1 " .. ZT("stack")
            elseif (numstacks == 0) then
                info = ZT("stacks of") .. " " .. stacksize
            elseif (numstacks > 0) then
                info = numstacks .. " " .. ZT("stacks of") .. " " .. stacksize
            end
...
function Atr_scanLevelDD_showTip(self)
    ...
    GameTooltip:AddLine(ZT("Only include items in the scanning database that are this level or higher"), 0.5, 0.5, 1.0,
        1)
    GameTooltip:Show()
end
...
function Atr_OnClick_ClearHistory(self)
    ...
    Atr_ClearConfirm_Text2:SetText(ZT(
        "This will clear the pricing history for all items for all your characters - even characters on different servers."))
...
function Atr_OnClick_ClearPostHistory(self)
    ...
    Atr_ClearConfirm_Text2:SetText(ZT(
        'This will clear the information that Auctionator keeps for all items that you\'ve posted - as shown on the "Other" tab after you scan for an item that you\'ve sold in the past.'))
...
function Atr_OnClick_ClearStackPrefs(self)
    ...
    Atr_ClearConfirm_Text2:SetText(ZT(
        "Go ahead - this isn't a big deal.  Auctionator will figure it out again fairly quickly.  This is just some info Auctionator keeps to help it set the default stack size a bit more intelligently."))
...
function Atr_OnClick_ClearShopLists(self)
    ...
    Atr_ClearConfirm_Text2:SetText(ZT(
        "If you put a lot of time into constructing detailed shopping lists, this will require you to buidl them all over again."))
            ...
            elseif (line ~= "") then
                ...
                if (itemCount > ATR_MAXNUM_ITEMS_ON_SHOPPING_LIST) then
                    Atr_Error_Display(ZT("Import failed.") .. "\n\n" ..
                            ...
                            ATR_MAXNUM_ITEMS_ON_SHOPPING_LIST))
                    return
                end
            end
        end
...
function Atr_OnClick_ShpList_Export(self)
    ...
    Atr_ShpList_Explanation:SetText(
        "Click Select All, type Ctrl-C to copy the text and then paste into any text document.")
...
function ShowInterfaceOptionsMask()
    if (gInterfaceOptionsMask == nil) then
        gInterfaceOptionsMask = CreateFrame("Frame", "Atr_Mask_StdOptions", _G["InterfaceOptionsFrame"],
            "Atr_Mask_StdOptionsTempl")

...
function Atr_OnChatMsgAddon(...)
    ...
    if (prefix == "ATR") then
        ...
        if (zc.StringStartsWith(msg, "IREQ_")) then
            ...
            SendAddonMessage("ATR",
                "I_" .. Atr_GetDBsize() .. "_" .. mem .. "_" .. #AUCTIONATOR_SHOPPING_LISTS .. "_" ..
                    GetRealmFacInfoString(), "WHISPER", sender)
        end
...
local function Atr_ModTradeSkillFrame()
    ...
    if (TradeSkillFrame) then
        ...
        --		button:SetPoint("TOPRIGHT", "TradeSkillFrameCloseButton", "TOPLEFT", 0, -8);
        local button = CreateFrame("BUTTON", "Auctionator_Search", TradeSkillDetailScrollChildFrame,
...
function Atr_OnLoad()
    ...
    if (AUCTIONATOR_OPEN_FIRST < 2) then -- set to 2 to indicate it's been migrated
        if (AUCTIONATOR_OPEN_FIRST == 1) then
            AUCTIONATOR_DEFTAB = 1
        elseif (AUCTIONATOR_OPEN_BUY == 1) then
    ...
    for bagID = 0, NUM_BAG_SLOTS do
        ...
        for slotID = 1, numslots do
            ...
            if (itemLink) then
                ...
                if (zc.StringContains(itemName, "Celestial Essence", "Heavenly Shard", "Hypnotic Dust",
                    "Maelstrom Crystal")) then
                    local key = itemName
                    if (mats[key]) then
...
function Atr_OnBagUpdate(...)
    ...
    if (numG == 1 and numM == 1) then
        ...
        local s =
            tm .. "_" .. result.matcount .. "_" .. matname .. "_" .. itemClassAbbrev .. "_" .. itemSubclass .. "_" ..
                result.q .. "_" .. result.lev
        --		zz (s);
...
function Atr_GetSellItemInfo()
    ...
    if (auctionItemName ~= "") then
        ...
        local hasCooldown, speciesID, level, breedQuality, maxHealth, power, speed, name =
...
function Atr_AuctionFrameTab_OnClick(self, index, down)
    ...
    if (not Atr_IsAuctionatorTab(index)) then
        ...
        if (AP_Bid_MoneyFrame) then -- for the addon 'Auction Profit'
            if (AP_ShowBid) then
                AP_ShowHide_Bid_Button(1)
            end
            if (AP_ShowBO) then
                AP_ShowHide_BO_Button(1)
            end
        end
    elseif (Atr_IsAuctionatorTab(index)) then
        ...
        end -- abort scan when changing tab
        if (index == Atr_FindTabIndex(SELL_TAB)) then
            gCurrentPane = gSellPane
        end
        ...
        if (index == Atr_FindTabIndex(MORE_TAB)) then
            gCurrentPane = gMorePane
        end

        if (index == Atr_FindTabIndex(SELL_TAB)) then
            AuctionatorTitle:SetText("Auctionator+ - " .. ZT("Sell"))
        end
        if (index == Atr_FindTabIndex(BUY_TAB)) then
            AuctionatorTitle:SetText("Auctionator+ - " .. ZT("Buy"))
        end
        if (index == Atr_FindTabIndex(MORE_TAB)) then
            AuctionatorTitle:SetText("Auctionator+ - " .. ZT("More") .. "...")
        end
...
function Atr_OnAuctionMultiSellUpdate(...)
    ...
    if (stacksSoFar == stacksTotal) then
        ...
        Atr_AddHistoricalPrice(gJustPosted.ItemName, gJustPosted.BuyoutPrice / gJustPosted.StackSize,
            gJustPosted.StackSize, gJustPosted.ItemLink)
...
function Atr_OnAuctionMultiSellFailure()
    ...
    Atr_AddHistoricalPrice(gJustPosted.ItemName, gJustPosted.BuyoutPrice / gJustPosted.StackSize, gJustPosted.StackSize,
        gJustPosted.ItemLink)
    ...
    if (gAtr_SellTriggeredByAuctionator) then
        if (gJustPosted.ItemName) then
            if (gJustPosted.NumStacks == 1) then
                ...
                Atr_AddHistoricalPrice(gJustPosted.ItemName, gJustPosted.BuyoutPrice / gJustPosted.StackSize,
                    gJustPosted.StackSize, gJustPosted.ItemLink)
                Atr_AddToScan(gJustPosted.ItemLink, gJustPosted.ItemName, gJustPosted.StackSize,
                    gJustPosted.BuyoutPrice, 1)
...
function Atr_ResetDuration()
    ...
    if (AUCTIONATOR_DEF_DURATION == "S") then
        UIDropDownMenu_SetSelectedValue(durMenu, 1)
    end
    if (AUCTIONATOR_DEF_DURATION == "M") then
        UIDropDownMenu_SetSelectedValue(durMenu, 2)
    end
    if (AUCTIONATOR_DEF_DURATION == "L") then
        UIDropDownMenu_SetSelectedValue(durMenu, 3)
    end
end
...
function Atr_UpdateRecommendation(updatePrices)
    ...
    elseif (scn.absoluteBest and basedata.stackSize == scn.absoluteBest.stackSize and basedata.buyoutPrice ==
        scn.absoluteBest.buyoutPrice) then
        Atr_Recommend_Basis_Text:SetText("(" .. ZT("based on cheapest current auction") .. ")")
    elseif (cheapestStack and basedata.stackSize == cheapestStack.stackSize and basedata.buyoutPrice ==
        cheapestStack.buyoutPrice) then
...
function Atr_StackSizeChangedFunc()
    ...
    if gCurrentPane.totalItems > 0 then
        local lowerstacknum = math.min(math.floor(gCurrentPane.totalItems / Atr_Batch_NumAuctions:GetNumber()),
            gCurrentPane.fullStackSize, gCurrentPane.totalItems)
...
function Atr_UpdateUI_SellPane(needsUpdate)
    ...
    if (needsUpdate) then
        ...
        if (gCurrentPane.totalItems > 1) then
            maxStackedItems = math.min(math.floor(gCurrentPane.totalItems / Atr_Batch_NumAuctions:GetNumber()),
                gCurrentPane.fullStackSize, gCurrentPane.totalItems)
        end
        ...
        if (gCurrentPane.activeSearch and gCurrentPane.activeSearch.processing_state ~= KM_NULL_STATE) then
            ...
            Atr_StartingPriceDiscountText:SetText(ZT("Starting Price Discount") .. ":  " ..
                                                      AUCTIONATOR_SAVEDVARS.STARTING_DISCOUNT .. "%")
            ...
            if (Atr_Batch_NumAuctions:GetNumber() < 2) then
                ...
                Atr_CreateAuctionButton:SetText(string.format(ZT("Create %d Auctions"),
                    Atr_Batch_NumAuctions:GetNumber()))
            end
        ...
        if (gJustPosted.ItemName ~= nil) then
            ...
            gCurrentPane.currIndex = gCurrentPane.activeScan:FindInSortedData(gJustPosted.StackSize,
                gJustPosted.BuyoutPrice)
...
function Atr_SetDepositText()
    ...
    if (auctionCount > 0) and (Atr_StackSize() > 0) and (Atr_Batch_NumAuctions:GetNumber() > 0) then
        ...
        Atr_Deposit_Text:SetText(ZT("Deposit") .. ":    " .. zc.priceToMoneyString(deposit1 * Atr_StackSize(), true) ..
                                     numAuctionString)
    else
...
function Atr_ShowingSearchSummary()
    if (gCurrentPane.activeSearch and gCurrentPane.activeSearch.searchText ~= "" and gCurrentPane:IsScanNil() and
        gCurrentPane.activeSearch:NumScans() > 0) then
        return true
    end
...
function Atr_FindBestCurrentAuction()
    ...
    if (Atr_IsModeCreateAuction()) then
        gCurrentPane.currIndex = scan:FindCheapest()
    elseif (Atr_IsModeBuy()) then
        gCurrentPane.currIndex = scan:FindCheapest()
    else
        gCurrentPane.currIndex = scan:FindMatchByYours()
    end
end
...
function Atr_HighlightEntry(entryIndex)
    ...
    if (Atr_IsSelectedTab_Current() and entryIndex ~= nil and entryIndex > 0 and entryIndex <=
        #gCurrentPane.activeScan.sortedData) then
...
function Atr_Set_StackingPrefs_numstacks(key, numstacks)
    ...
    if (not Atr_Has_StackingPrefs(lkey)) then
        AUCTIONATOR_STACKING_PREFS[lkey] = {
            stacksize = 0
        }
    end
...
function Atr_IsAuctionatorTab(tabIndex)
    if (tabIndex == Atr_FindTabIndex(SELL_TAB) or tabIndex == Atr_FindTabIndex(MORE_TAB) or tabIndex ==
        Atr_FindTabIndex(BUY_TAB)) then
        return true
    end
    ...
    for x = #scan.sortedData, 1, -1 do
        ...
        if (data.yours and data.itemPrice > scan.absoluteBest.itemPrice) then
            if (not confirmed) then
                ...
                Atr_CancelAuction_Confirm_Frame_text:SetText(
                    string.format(ZT("Your auction has been undercut:\n%s%s"), "|cffffffff", scan.itemName))
...
function Atr_ResetMassCancel()
    ...
    Atr_CheckActives_Text:SetText(string.format(ZT(
        "%d of your auctions are not the lowest priced.\n\nWould you like to cancel them?"), #gAtr_MassCancelList))
...
function Atr_CalcUndercutPrice(price)
    if (price > 5000000) then
        return roundPriceDown(price, AUCTIONATOR_SAVEDVARS._5000000)
    end
    if (price > 1000000) then
        return roundPriceDown(price, AUCTIONATOR_SAVEDVARS._1000000)
    end
    if (price > 200000) then
        return roundPriceDown(price, AUCTIONATOR_SAVEDVARS._200000)
    end
    if (price > 50000) then
        return roundPriceDown(price, AUCTIONATOR_SAVEDVARS._50000)
    end
    if (price > 10000) then
        return roundPriceDown(price, AUCTIONATOR_SAVEDVARS._10000)
    end
    if (price > 2000) then
        return roundPriceDown(price, AUCTIONATOR_SAVEDVARS._2000)
    end
    if (price > 500) then
        return roundPriceDown(price, AUCTIONATOR_SAVEDVARS._500)
    end
    if (price > 0) then

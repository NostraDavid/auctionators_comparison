...
function AtrReadBindText(itemID, txt)
    ...
    if (txt) then
        if txt == ITEM_BIND_ON_EQUIP then
            return ATR_CAN_BE_AUCTIONED
        end
        if txt == ITEM_BIND_ON_USE then
            return ATR_CAN_BE_AUCTIONED
        end
        if txt == ITEM_BIND_ON_PICKUP then
            return ATR_BIND_ON_PICKUP
        end
        if txt == ITEM_BIND_TO_ACCOUNT then
            return ATR_BINDS_TO_ACCOUNT
        end
        if txt == ITEM_BIND_TO_BNETACCOUNT then
            return ATR_BINDS_TO_ACCOUNT
        end
        if txt == ITEM_BIND_QUEST then
            return ATR_QUEST_ITEM
        end
    end

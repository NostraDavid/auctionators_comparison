...
function AtrSearch:Start()
    ...
    if (self.exactMatchtext == nil and not self.IDstring and not Atr_IsCompoundSearch(self.searchText) and
        string.len(self.searchText) > 2) then
        ...
        for x = 1, numBatchAuctions do
            ...
            if (itemLink) then
                ...
                if (self.minItemLevel or self.maxItemLevel) then
                    ...
                    if ((self.minItemLevel and iLevel < self.minItemLevel) or
                        (self.maxItemLevel and iLevel > self.maxItemLevel)) then
                        OKitemLevel = false
                    end
                end
...
function AtrSearch:Continue()
    if (CanSendAuctionQuery()) then
        ...
        if (Atr_IsCompoundSearch(self.searchText)) then
            queryString, itemClass, itemSubclass, minLevel, maxLevel, _, _, qualityIndex = Atr_ParseCompoundSearch(
                self.searchText)
        ...
        QueryAuctionItems(queryString, minLevel, maxLevel, nil, itemClass, itemSubclass, self.current_page, nil,
            qualityIndex)
...
function Atr_MigtrateMaxHistAge() -- 21 was too much
    if (AUCTIONATOR_DB_MAXHIST_AGE and AUCTIONATOR_DB_MAXHIST_AGE ~= 21 and AUCTIONATOR_DB_MAXHIST_AGE ~= -1) then
        zz("AUCTIONATOR_DB_MAXHIST_AGE:", AUCTIONATOR_DB_MAXHIST_AGE, "AUCTIONATOR_DB_MAXHIST_DAYS:",

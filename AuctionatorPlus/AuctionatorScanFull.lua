...
function Atr_FullScanFrameIdle()
    ...
    if (gAtr_FullScanState == ATR_FS_STARTED) then
        statusText = "Waiting for auction data"
    end
...
function Atr_ShowFullScanFrame()
    ...
    local expText = "<html><body>" .. "<p>" .. ZT("SCAN_EXPLANATION") .. "</p>" .. "</body></html>"
...
function Atr_FullScanMoreDetails()
    zc.msg(" ")
    zc.msg_anm(ZT("Auctions scanned") .. ": |cffffffff", gScanDetails.numBatchAuctions,
        " |r(" .. gScanDetails.totalItems, "items) ", "time: ", Atr_FullScan_GetDurString())
    ...
    if (gScanDetails.numRemoved[4] > 0) then
        zc.msg_anm(ZT("Rare items") .. " " .. ZT("removed from database") .. ": |cffffffff", gScanDetails.numRemoved[4])
    end
    if (gScanDetails.numRemoved[3] > 0) then
        zc.msg_anm(ZT("Uncommon items") .. " " .. ZT("removed from database") .. ": |cffffffff",
            gScanDetails.numRemoved[3])
    end
    if (gScanDetails.numRemoved[2] > 0) then
        zc.msg_anm(ZT("Common items") .. " " .. ZT("removed from database") .. ": |cffffffff",
            gScanDetails.numRemoved[2])
    end
    if (gScanDetails.numRemoved[1] > 0) then
        zc.msg_anm(ZT("Poor items") .. " " .. ZT("removed from database") .. ": |cffffffff", gScanDetails.numRemoved[1])
    end
    ...
    if (gFSNumNullItemNames > 0) then
        zc.msg_anm(string.format("|cffff3333%d auctions returned empty results (out of %d)|r", gFSNumNullItemNames,
            gScanDetails.numBatchAuctions))
    end

    if (gFSNumNullItemLinks > 0) then
        zc.msg_anm(string.format("|cffff3333%d auctions returned null itemLinks (out of %d)|r", gFSNumNullItemLinks,
            gScanDetails.numBatchAuctions))
    end

    if (not gGetAllSuccess) then
        zc.msg(" ")
        zc.msg_anm("|cffff3333Warning:|r Blizzard server failed to return all items: ", gGetAllTotalAuctions,
            gGetAllNumBatchAuctions)

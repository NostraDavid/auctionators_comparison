function AtrBuildLTable_ruRU()
    ...
    AtrL["SCAN_EXPLANATION"] =
        "Во время сканирование аукциона создаётся база данных по ценам, которую Auctionator будет использовать в двух целях:" ..
            ...
            " аукцион только один раз каждые 15 минут. " .. "<br/><br/>" ..
    ...
    AtrL["Auctionator has yet to record any auctions for this item"] =
        "Auctionator до сих пор не регистрировал каких-либо аукционов для этого предмета"
    AtrL["Auctionator provided an auction module to LilSparky's Workshop."] =
        "Auctionator предоставяет LilSparky's Workshop как модуль аукциона."
    ...
    AtrL["Automatically cancel all of your auctions|n|nthat are not the lowest priced?"] =
        "Автоматически отменить все ваши аукционы|n|nкоторых цена не самая низкая?"
    ...
    AtrL["based on cheapest current auction"] =
        "основанно на текущих дешевых аукционах"
    AtrL["based on cheapest stack of the same size"] =
        "основанно на текущих дешевых стопках схожего размера"
    ...
    AtrL["Drag an item you want to sell to this area."] =
        "Перетащите в данную область предмет, который желаете продать."
    ...
    AtrL["For information on the latest version browse to"] =
        "Для получения информации о последней (Eng) версии посетите"
    ...
    AtrL["full scan database cleared"] =
        "база данных полного сканирования - очищена"
    ...
    AtrL["If this option is checked, ALL your bags will be opened when you first open the Auctionator panel."] =
        "Если этот пункт отмечен, то при открытии панель Auctionatorа все ваши сумки также раскроются."
    AtrL["If this option is checked, every time you initiate a new auction the auction duration will be reset to the default duration you've selected."] =
        "Если этот пункт отмечен, то каждый раз, когда вы объявите новый аукцион, длительность аукциона будет сбрасываться к значениям по умолчанию что вы выбрали."
    AtrL["If this option is checked, holding the Alt key down while clicking an item in your bags will switch to the Auctionator panel, place the item in the Auction Item area, and start the scan."] =
        "Если этот пункт отмечен, удерживая нажатой клавишу Alt и кликнув по предмету в ваших сумках, вы перейдёте на панель Auctionatorа, выставив предмет по которому кликнули, в окно выставки на аукцион, и в тоже время просканируете цены."
    AtrL["If this option is checked, the Auctionator BUY panel will display first whenever you open the Auction House window."] =
        "Если этот пункт отмечен, то при открытии аукционного дома, панель покупок Auctionatorа будет отображаться в первую очередь."
    AtrL["Ignore any ERROR message to the contrary below."] =
        "	Игнорировать любые сообщения об ошибках."
    ...
    AtrL["Really delete the shopping list %s ?"] =
        "Действительно удалить cписок закупок - %s ?"
    ...
    AtrL["Select the Auctionator panel to be displayed first whenever you open the Auction House window."] =
        "Выберите какое окно будет открываться по умолчанию при открытии окна аукциона."
    AtrL["Sell"] = "Продать"
    ...
    AtrL["The latest information on Auctionator can be found at"] =
        "Самую последнюю информацию о Auctionatorе можно найти по адресу"
    AtrL["There is a more recent version of Auctionator: VERSION"] =
        "Существует более новая версия: VERSION"
    ...
    AtrL["You can stack at most %d of these items"] =
        "Вы можете складывать в пачку не больше %d шт таких предметов"
    ...
    AtrL["You may have at most 40 single-stack (x1)\nauctions posted for this item.\n\nYou already have %d such auctions and\nyou are trying to post %d more."] =
        "Вы можете иметь не более 40 однорядные (x1) аукционов относительно этого пункта. У вас уже есть% D таких аукционов, а вы пытаетесь на должность D% больше."
    AtrL["You may have no more than\n\n%d items on a shopping list."] =
        "Вы можете иметь не более\n\n%d предметов в вашем списке закупок"
    AtrL["your auction on"] = "ваш аукцион"
    AtrL["your most recent posting"] = "последняя запись"
    AtrL["yours"] = "ваши"

    -- Auctionator.lua
    AtrL["Unknown datatype:"] = "Неизвестный тип данных:" -- 3396 --AuctionatorConfig.lua
    --
...
"stack" --3361
    ...
    AtrL["Show Starting Price on the Sell Tab"] =
        "Показывать начальную цену во вкладке продаж"
    AtrL["Selling"] = "Продажа"
    AtrL["Configure how you typically like to sell the items listed below."] =
        "Тут вы можете настроить как продавать предмети преведённые ниже."
    AtrL["Default Auctionator tab"] = "Окно по умолчанию"
    AtrL["Show auction house prices in tooltips"] =
        "Отображать в подсказках цену на аукционе"
    AtrL["Show disenchant prices in tooltips"] =
        "Отображать в подсказках стоимость распыления"

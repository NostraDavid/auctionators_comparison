...
function Atr_GetMeanPrice(item) -- itemName or itemID
	...
	if (gAtr_MeanDB and gAtr_MeanDB[itemName] and #gAtr_MeanDB[itemName] > 0) then
		...
		if #gAtr_MeanDB[itemName] % 2 == 0 then
			median =
				(gAtr_MeanDB[itemName][#gAtr_MeanDB[itemName] / 2] + gAtr_MeanDB[itemName][#gAtr_MeanDB[itemName] / 2 + 1]) / 2
		else
				...
				zc.msg_pink(
					"   " ..
						ta[x] ..
							"%  " ..
								ta[x + 1] ..
									"  " .. Atr_GetDEitemName(ta[x + 2]) .. "  (" .. Atr_GetAuctionPrice(Atr_GetDEitemName(ta[x + 2])) .. ")"
				)
			end
		end
	end

...
function Atr_GetMeanPrice(item) -- itemName or itemID
    ...
    if (gAtr_MeanDB and gAtr_MeanDB[itemName] and #gAtr_MeanDB[itemName] > 0) then
        local median = nil
        if #gAtr_MeanDB[itemName] % 2 == 0 then
            median = (gAtr_MeanDB[itemName][#gAtr_MeanDB[itemName] / 2] +
                         gAtr_MeanDB[itemName][#gAtr_MeanDB[itemName] / 2 + 1]) / 2
        else
            median = gAtr_MeanDB[itemName][math.ceil(#gAtr_MeanDB[itemName] / 2)]
        end
...
function Atr_DumpDETable(itemType, itemRarity)
    ...
    if (t) then
        ...
        for n = 1, #t do
            ...
            for x = 3, #ta, 3 do
                zc.msg_pink("   " .. ta[x] .. "%  " .. ta[x + 1] .. "  " .. Atr_GetDEitemName(ta[x + 2]) .. "  (" ..
                                Atr_GetAuctionPrice(Atr_GetDEitemName(ta[x + 2])) .. ")")
            end
        end
    end
end
...
local function ShowTipWithPricing(tip, link, num)
    ...
    local itemName, itemLink, itemRarity, itemLevel, itemMinLevel, itemType, _, _, _, _, itemVendorPrice = GetItemInfo(
        link)
    ...
    if (AUCTIONATOR_A_TIPS == 1) then
        auctionPrice = Atr_GetAuctionPrice(itemName)
    end
    if (AUCTIONATOR_A_TIPS == 1) then
        auctionMedianPrice = Atr_GetMeanPrice(itemName)
    end
    if (AUCTIONATOR_D_TIPS == 1) then
        dePrice = Atr_CalcDisenchantPrice(itemType, itemRarity, itemLevel)
    end
    ...
    if (num and showStackPrices) then
        if (auctionPrice) then
            auctionPrice = auctionPrice * num
        end
        if (auctionMedianPrice) then
            auctionMedianPrice = auctionMedianPrice * num
        end
        if (vendorPrice) then
            vendorPrice = vendorPrice * num
        end
        if (dePrice) then
            dePrice = dePrice * num
        end
    ...
    if (AUCTIONATOR_DE_DETAILS_TIPS == 1) then
        showDetails = IsShiftKeyDown()
    end
    if (AUCTIONATOR_DE_DETAILS_TIPS == 2) then
        showDetails = IsControlKeyDown()
    end
    if (AUCTIONATOR_DE_DETAILS_TIPS == 3) then
        showDetails = IsAltKeyDown()
    end
    if (AUCTIONATOR_DE_DETAILS_TIPS == 4) then
        showDetails = false
    end
    if (AUCTIONATOR_DE_DETAILS_TIPS == 5) then
        showDetails = true
    end
